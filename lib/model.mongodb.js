...
function Map (id, db, item) {
  ...
  this._positions = db.collection(id + '_positions')

  this.slug = item.slug
  this.tiles = item.tiles
}

Map.prototype.importGeoJSON = function importGeoJSON (geojson, options, callback) {
  var id = this.id
  var positions = this._positions
  var trajectories = this._trajectories

  var trajectoryId = geojson._id

  var running = 0

  running += geojson.loc.features.length
  geojson.loc.features.forEach(function (feature, i) {
  ...
  running++
  trajectories.insert(geojson, function (err) {
    if (err) {
      throw err
      return
    }
...
Map.prototype.createIndex = function createIndex (options, callback) {
  var id = this.id
  var positions = this._positions
  var trajectories = this._trajectories
...
Map.prototype.findTrajectories = function findTrajectories (query, callback) {
  ...
  positions.distinct('trajectory', query, function (err, ids) {
    if (err) {
      console.error(err)
    }
